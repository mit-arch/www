...
// the term's standing slot, which is written once under `term`.
//
...
function buildYaml({ term, weekday, time, room, host, weeks, startTimestamp }) {
  // The slot is written once, on the term. Each row carries only what varies
  // week to week; src/_data/seminarTerms.js folds the slot back into every talk.
  const talks = Array.from({ length: weeks }, (_, week) => {
    const date = formatDate(startTimestamp + week * 7 * 86400000);

    return [`  - date: ${date}`, `    speaker: TBD`, `    title: TBD`].join("\n");
  });

  return `${[
    `# Generated by scripts/new-seminar.js.`,
    `# ${weeks} weekly slot${weeks === 1 ? "" : "s"} from ${formatDate(startTimestamp)}.`,
    `# Day, time, and room are the term's standing slot and are stated once under`,
    `# "term" below; a talk only needs a date, speaker, title, and (optionally) host.`,
    `# Add "cancelled: true" with a title to keep a skipped week on the schedule, or`,
    `# a per-talk "time"/"location" for the odd week that moves.`,
    `term:`,
    `  slug: ${scalar(term.slug)}`,
    `  title: ${scalar(term.title)}`,
    `  host: ${scalar(host)}`,
    `  day: ${scalar(weekday.plural)}`,
    `  time: ${scalar(time)}`,
